...
// void check_key(void (*callback)(void)) {
//   // read new character from serial, it we have a CR or NL, then we have a line, so run the callback
...
//     } else if (c>=32 && c<=255) {
//       key_buffer[next_posn] = c;
//       key_buffer[next_posn+1] = '\0';
//       next_posn++;
//       if (next_posn >= sizeof(key_buffer)-1) {
//         next_posn = 0;
//       }
//     }
//   }
...
// void parse_light_level_serial_input()
// {
//   // When we get an input we'll split it into the 4 numbers (using leading zeros so each is 4 digits) and set each light bank level
//   if (next_posn < 19 || next_posn > 20) {
//     Serial.printf("\nFailed to parse: %s\n",key_buffer);
//     Serial.println("\nInput too short, need 19 digits (4 per bank) i.e.\n\t0000 0000 0000 0000");
//     next_posn=0;
//      memset(key_buffer, 0, sizeof(key_buffer));
//     return;
//   }
//   Serial.printf("\nSetting light levels from serial input\n");
//   for (uint8_t i=0;i<4;i++)
//   {
//     uint16_t value=atoi(&key_buffer[i*5]);
//     Serial.printf("\t\tBank %d -> level %d\n",i,value);
//     bank_levels[i]=value;
//     ledcWrite(bank_pios[i],value);
//   }
//   next_posn=0;
//      memset(key_buffer, 0, sizeof(key_buffer));
...
//       level=0;
//     }
//     ledcWrite(bank_pios[bank_no],bank_levels[level]);
